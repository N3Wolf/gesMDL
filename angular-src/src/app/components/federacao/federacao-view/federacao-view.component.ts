...
@Component({
  ...
  templateUrl: './federacao-view.component.html',
  styleUrls: ['./federacao-view.component.css']
})
  ...
  onFederacaoSubmit() {
    ...
    if(this.isInsert){
      this.federacaoService.addFederacao(newFederacao).subscribe(data => {
        if (data.success) {
          //console.log(data);
          this.router.navigate(['federacao-view', { id: data.id, isView: true }]);
      ...
      this.federacaoService.updateFederacao(this.Federacao).subscribe(data => {
        if (data.success) {
          //console.log(data);
          this.router.navigate(['federacao-view', { id: this.Federacao._id, isEdit: true }]);
